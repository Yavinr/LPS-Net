...
def triplet_loss(q_vec, pos_vecs, neg_vecs, margin, use_min=False, lazy=False,
...
def triplet_loss_wrapper(q_vec, pos_vecs, neg_vecs, other_neg, m1, m2,
                         use_min=False, lazy=False, ignore_zero_loss=False):
    return triplet_loss(q_vec, pos_vecs, neg_vecs, m1, use_min, lazy,
                        ignore_zero_loss)


def quadruplet_loss(q_vec, pos_vecs, neg_vecs, other_neg, m1, m2, use_min=False,
                    lazy=False, ignore_zero_loss=False, soft_margin=False):
    min_pos, max_pos = best_pos_distance(q_vec, pos_vecs)
    ...
    if ignore_zero_loss:  # false
        ...
        triplet_loss_ = triplet_loss.mean()
    ...
    if ignore_zero_loss:
        ...
        second_loss_ = second_loss.mean()

    total_loss = triplet_loss + second_loss
    total_loss_ = triplet_loss_ + second_loss_

    return total_loss_, total_loss


def contrastive_quadruplet_loss(q_vec, pos_vecs, neg_vecs, other_neg, m1, m2,
                                use_min=False, lazy=True,
...
def hphn_quadruplet_loss(q_vec, pos_vecs, neg_vecs, other_neg, m1, m2,
                         use_min=False, lazy=False, ignore_zero_loss=False):
    ...
    hard_neg = \
        torch.stack([min_neg, min_other_neg], dim=1).min(dim=1, keepdim=False)[
            0]
...
def nearest_neighbor(src, dst):
    inner = -2 * torch.matmul(src.transpose(2, 1).contiguous(),
                              dst)  # src, dst (num_dims, num_points)
    distances = -torch.sum(src ** 2, dim=1, keepdim=True).transpose(2,
                                                                    1).contiguous() - inner - torch.sum(
        dst ** 2,
        dim=1,
        keepdim=True)
