...
def construct_query_dict(df_centroids, filename):
    ...
    for i in range(len(ind_nn)):
        ...
        array([1, 2])
        """
        # np.setdiff1d: get difference   less than 10m
...
for folder in folders:
    df_locations = pd.read_csv(
        os.path.join(base_path, runs_folder, folder, filename), sep=',')  # filename = "pointcloud_locations_20m_10overlap.csv"
    ...
    for index, row in tqdm(df_locations.iterrows()):
        if (check_in_test_set(row['northing'], row['easting'], p, x_width,
                              y_width)):
