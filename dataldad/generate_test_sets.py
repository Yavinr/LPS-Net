...
def construct_query_and_database_sets(base_path, runs_folder, folders, pointcloud_fols, filename, p, output_name):
    ...
    for i in range(len(database_sets)):
        ...
        for j in range(len(test_sets)):
            if (i == j):
                continue  # 对每个文件，都分别取除该文件以外的所有test文件
            for key in range(len(test_sets[j].keys())):  # 对（第i个文件以外的）每个test文件内的每一个bin文件
                coor = np.array([[test_sets[j][key]["northing"], test_sets[j][key]["easting"]]])  # 取出xy坐标
                index = tree.query_radius(coor, r=25)  # 这里的tree是第i个文件的database_tree，在第i个文件的所有bin文件中，找距离小于25的点
